...
class StateNormalization(object):
    def __init__(self):
        ...
        self.high_state = np.append(self.high_state, np.ones(M) * 2097153)
        ...
        self.low_state = np.zeros_like(self.high_state)
        self.low_state[len(self.low_state) - 2 * M:len(self.low_state) - M] = np.ones(M) * 2097153

    def state_normal(self, state):
        state[len(state) - 2 * M: len(state) - M] -= 2097153
