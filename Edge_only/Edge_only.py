...
class UAVEnv(object):
    ...
    task_list = np.random.randint(int(1.5 * 1024 * 1024), int(2 * 1024 * 1024) + 1, M)  # 随机计算任务1.5~2Mbits
    # ue位置转移概率
    ...
    for k in range(10):
        ...
        while i < env.slot_num:
            ...
            if is_terminal or i == env.slot_num - 1:
                ...
                break
            i = i + 1
    mean_delay = np.around(np.mean(delays_list), 4)
    return mean_delay
...
def plot_local_only():
    ...
    # 生成横轴 Episodes（假设 1000 轮）
    episodes = np.arange(0, 1000, 10)
    # 纵轴是一个常数水平线
    local_only = np.ones_like(episodes) * avg_delay

    # 绘图
    plt.figure(figsize=(7, 5))
    plt.plot(episodes, local_only, '+-', color='magenta', label='Local-only', linewidth=1.2, markersize=5)
